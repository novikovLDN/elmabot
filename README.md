...
- 🚀 Брендированная страница подключения `web/connect.html` — отдаётся самим
  сервисом по маршруту **`/connect`** (отдельный хостинг не нужен). Кнопка
  «Открыть в Happ» авто-открывает приложение и импортирует подписку; ключ
  передаётся в URL `#fragment` и на веб-сервер не попадает. По умолчанию
  `CONNECT_PAGE_URL` сам выводится из публичного домена (`RAILWAY_PUBLIC_DOMAIN`
  → `https://<домен>/connect`), так что одно-нажатное подключение работает из
  коробки на webhook-деплое. Можно переопределить `CONNECT_PAGE_URL`, чтобы
  хостить страницу отдельно. Если базового URL нет (локальный polling) — кнопка
  показывает ключ текстом.
...
- 📢 Рассылка (`app/services/broadcaster.py`): фото с подписью или текст с
  ...
  - 🎯 Сегменты: все · активная подписка (все) · 💳 платная (активная) · 🆓 триал
    (активный) · холодные (без триала и подписки) · триал без покупки · триал
    истёк 3+ дн. без покупки · без подписок вообще · ⏳ истекает через 1–7 дней ·
    🔚 закончилась 1–7 дней назад. Посуточные бакеты — по статусу/`expires_at`
    (активные для напоминаний, истёкшие для win-back); списки дней настраиваются
    в `EXPIRY_DAY_BUCKETS` / `EXPIRED_DAY_BUCKETS`.
  - 🔘 Кнопки к сообщению: «Купить со скидкой» (админ задаёт % и срок действия;
    ...
    персональный оффер), «Перейти в канал» (`CHANNEL_URL`, иначе заглушка) и
    «Пригласить друга» (открывает получателю экран реферальной программы).
