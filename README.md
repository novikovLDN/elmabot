...
- 🚀 Брендированная страница подключения `web/connect.html` (статический файл,
  залить на свой домен/хостинг). Кнопка «Открыть в Happ» авто-открывает
  приложение и импортирует подписку; ключ передаётся в URL `#fragment` и на
  веб-сервер не попадает. Включается переменной `CONNECT_PAGE_URL`
  (напр. `https://your-domain/connect.html`) — тогда на экранах устройств
  появляется кнопка «🚀 Открыть в Happ». Не задана — показываем ключ текстом.
...
- 🛠 `/admin` (только `ADMIN_TELEGRAM_ID` из env): дашборд (новые юзеры,
